...
public class HttpBlockerClient implements BlockerClient {
    ...
    private OperationCheckResponse checkWithoutCircuitBreaker(OperationCheckRequest request) {
        try {
            if (log.isDebugEnabled()) {
                log.debug(
                        "Blocker downstream request prepared operationId={} operationType={} currency={} source=cash-service targetService=blocker-service",
                        request.operationId(),
            ...
            log.error(
                    "Blocker downstream request failed operationId={} operationType={} currency={} status=error errorCategory=downstream_unavailable errorType={} source=cash-service targetService=blocker-service",
                    request.operationId(),
                    request.operationType(),
                    request.currency(),
    ...
    private OperationCheckResponse blockerFallback(Throwable exception) {
        ...
        log.error(
                "Blocker downstream retries exhausted status=error errorCategory=downstream_unavailable errorType={} source=cash-service targetService=blocker-service",
                exception.getClass().getSimpleName()
        );
